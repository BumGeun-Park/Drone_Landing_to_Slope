...
public:
	SubscribeAndPublish()
	{
	pub_ = n_.advertise<rplidar_data::alpha>("/error2",1);
	sub_1 = n_.subscribe("/dist2",1,&SubscribeAndPublish::callback1,this);
	sub_2 = n_.subscribe("/avg_z",1,&SubscribeAndPublish::callback2,this);
	}
	
	void callback1(const rplidar_data::alpha& dist2)
	{
	  Dist = dist2.alpha;
