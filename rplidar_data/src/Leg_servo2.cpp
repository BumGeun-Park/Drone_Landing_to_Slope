...
public:
        SubscribeAndPublish()
        {
        pub_ = n_.advertise<rplidar_data::alpha>("/Leg2_z",1);
	...
	void callback(const sensor_msgs::JointState& input)
	  {
	    ...
	    output.alpha = Leg_length + Arm_length*tan(DEG2RAD(start_deg)+input.position[Leg2]); // ID3(다리 모터)
	    pub_.publish(output);
	  }
