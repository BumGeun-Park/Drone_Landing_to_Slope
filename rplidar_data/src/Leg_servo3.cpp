...
public:
        SubscribeAndPublish()
        {
        pub_ = n_.advertise<rplidar_data::alpha>("/Leg3_z",1);
	...
	void callback(const sensor_msgs::JointState& input)
	  {
	    ...
	    output.alpha = Leg_length + Arm_length*tan(DEG2RAD(start_deg)+input.position[Leg3]); // ID4(다리 모터)
	    pub_.publish(output);
	  }
