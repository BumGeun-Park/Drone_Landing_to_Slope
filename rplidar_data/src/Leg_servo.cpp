	...
	void callback(const sensor_msgs::JointState& input)
	  {
            ...
            output.packet[0] = Leg_length + Arm_length*tan(input.position[Leg1]); // ID2(다리 모터)
            output.packet[1] = Leg_length + Arm_length*tan(input.position[Leg2]); // ID3(다리 모터)
            output.packet[2] = Leg_length + Arm_length*tan(input.position[Leg3]); // ID4(다리 모터)
            output.packet[3] = Leg_length + Arm_length*tan(input.position[Leg4]); // ID5(다리 모터)
	    pub_.publish(output);
	  }
